...
float msmall(vector<point> allp, double left, double right)
{ 
    ...
    double ll = msmall(allp, left, mid);
    double rr = msmall(allp, mid + 1, right);
...
void part2()
{
    ...
    msmall(allp, 0, allp.size() - 1);
    
    
    x = jax;
    y = jay;
    cout << "Closest Points: " <<  x.toS()<< " and "<< y.toS()<< " Distance: "<<ideal<<endl;
...
int main()
{
        ...
        gfile << "(Brute Force) Closest Points: " <<  iax.toS()<< " and "<< iay.toS()<< " Distance: "<<ideal1<<endl;
        gfile << "elapsed time: " << t1 << "s\n";
    
        gfile << "(Recursive) Closest Points: " <<  jax.toS()<< " and "<< jay.toS()<< " Distance: "<<ideal<<endl;
