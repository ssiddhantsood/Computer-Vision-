...
void quickHull(vector<point> &allp)
{
    ...
    ppmFinishUp("quickhull.ppm");
    
}


int grahamFindIndex(vector<point> &allp)
{
    int index = 0;
    for(int i = 0;i<int(allp.size());i++)
    {
        if (allp[i].getX() == allp[index].getX())
        {
            if(allp[i].getY()<allp[index].getY())
            {
                index = i;
            }
        }
        else if (allp[i].getX()<allp[index].getX())
        {
            index = i;
        }
    }
...
vector<point> genTemp(vector<point> &allp,int index)
{
    vector<point> temp;
     for(int i = 0; i<int(allp.size());i++)
    {
        if((allp[i].getX() != allp[index].getX()) && (allp[i].getY() != allp[index].getY()))
        {
            temp.push_back(allp[i]);
        }
    }
    return temp;
}
...
void sortTemp(vector<point> &temp,int index,vector<point> &allp)
{
    sort(temp.begin(), temp.end(),[&](point ii, point jj)
    { 
        if (cp(ii,jj,allp[index]) ==  0)
        {
            return (dist(ii,point(0,0)) < dist(jj,point(0,0)));
        }
        return cp(ii,jj,allp[index])>0;
    });
...
vector<point> grahamScan(vector<point> &allp)
{

    int index = grahamFindIndex(allp);
    vector<point> temp = genTemp(allp,index);
    sortTemp(temp,index,allp);
    
    stack<point> convHull; 
    convHull.push(allp[index]);
    
    
    for(int i = 0; i< int(temp.size());i++)
    {
        while(convHull.size()>1)
        {
            point top = convHull.top();
            convHull.pop();
            point next = convHull.top();
            convHull.push(top);
            
            double cc = cp(next,top,temp[i]);
        ...
        convHull.push(temp[i]);
    }
    
    
    vector<point> ret;
    while(!(convHull.empty()))
    {
        ret.push_back(convHull.top());
        convHull.pop();   
    }  
...
void part2()
{
    ...
    vector<point> allp = rf("points.txt");
    vector<point> hull1 = grahamScan(allp);
    ...
    {
        int x1 = int(hull1[i].getX()*size);
        int y1 = int(hull1[i].getY()*size);
        
        int x2 = int(hull1[(i + 1) % hull1.size()].getX()*size);
        int y2 = int(hull1[(i + 1) % hull1.size()].getY()*size);
